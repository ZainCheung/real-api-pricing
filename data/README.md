...
Current snapshot: 2026-09-10. The adopted dataset contains 201 plan × model rows: 190 subscription rows and 11 metered API rows. This covers the project's adopted sample, not every plan or model on the market.
...
当前采用数据共201条“套餐 × 模型”：190条订阅、11条按量API，包括OpenCode Go 28个模型、Command Code GOAT 39个模型、Ollama Pro/Max各16个模型。所有采用数据都参与对应的全量输出；缺榜单分数的模型不进入该榜帕累托图，但仍保留在额度和单价数据中。
...
|---|---:|---:|
| Code Arena | 136 / 201 | 65 |
| Agent Arena | 140 / 201 | 61 |
| AA Intelligence | 171 / 201 | 30 |
| AA Coding Agent | 73 / 201 | 128 |
