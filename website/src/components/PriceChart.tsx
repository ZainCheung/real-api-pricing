  ...
  return (
    <div className="card p-5 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="text-xl font-semibold text-ink">{t('priceTitle')}</h3>
          <p className="mt-1 max-w-2xl text-sm text-ink-muted">{t('priceSub')}</p>
        </div>
                ...
                return (
                  <ChartTooltipShell active={active}>
                    ...
                    <div className="num mt-1 text-accent">
                      {t('tooltipPrice')}: ${row.value.toPrecision(4)}
                    </div>
                  </ChartTooltipShell>
                )
              }}
            ...
            <Bar dataKey="value" radius={[0, 4, 4, 0]} maxBarSize={14}>
              {data.map((d) => (
                <Cell
                  key={d.id}
                  fill={d.color}
                  fillOpacity={d.billing === 'metered' ? 0.45 : 1}
                  stroke={d.billing === 'metered' ? d.color : undefined}
                  strokeWidth={d.billing === 'metered' ? 1 : 0}
                />
              ))}
            </Bar>
          </BarChart>
